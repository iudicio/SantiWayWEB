...
class AnomalyDetectionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Детали аномалии", {"fields": ("description", "device_data", "metadata")}),
        (
            "Временные метки",
    ...
    def get_queryset(self, request):
        return (
            ...
            .select_related("polygon_action__polygon", "resolved_by")
        )
    ...
    def retry_failed(self, request, queryset):
        ...
        for notification in queryset.filter(status="failed"):
            if notification.can_retry():
                ...
                retried += 1
        self.message_user(
            request, f"Поставлено на повторную отправку: {retried} уведомлений"
        )
