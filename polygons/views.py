...
class IsOwner(permissions.BasePermission):
    def has_object_permission(self, request, view, obj):
        return getattr(obj, "user_id", None) == getattr(
    ...
    def perform_destroy(self, instance):
        ...
        with transaction.atomic():
            active_actions = PolygonAction.objects.filter(
                polygon=instance, status__in=["running", "pending", "paused"]
            ).select_for_update()
    ...
    def search(self, request, pk=None):
        ...
        try:
            ...
            devices = search_devices_in_polygon(
                polygon.geometry, user_api_key=api_key_str
            )
            return Response(
                {
    ...
    def start_monitoring(self, request, pk=None):
        ...
        try:
            ...
            task = monitor_mac_addresses.delay(
                str(polygon.id), api_key_str, monitoring_interval
            )
    ...
    def monitoring_status(self, request, pk=None):
        ...
        try:
            actions = PolygonAction.objects.filter(
                polygon=polygon, action_type="mac_monitoring"
            ).order_by("-created_at")
...
class AnomalyDetectionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user:
            return (
                AnomalyDetection.objects.filter(polygon_action__polygon__user=user)
...
class NotificationViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user:
            return (
                Notification.objects.filter(anomaly__polygon_action__polygon__user=user)
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            raise serializers.ValidationError(
                {
                    "non_field_errors": [
                        "Такая цель уже существует для этого действия полигона"
                    ]
                }
            )
