...
def send_notification_via_websocket(notification):
    ...
    try:
        ...
        async_to_sync(channel_layer.group_send)(
            user_group_name,
            {"type": "notification.alert", "notification": notification_data},
        )

        notification.mark_as_sent()

        logger.info(
            f"Notification {notification.id} sent via WebSocket to user {user.username}"
        )
        return True
...
def create_and_send_notifications(anomaly):
    ...
    for target in notification_targets:
        ...
        if target.target_type in ["api_key", "device"]:
            ...
            logger.info(
                f"Skipping notification for deprecated target type: {target.target_type}"
            )
            notification.mark_as_failed()
...
def retry_failed_notifications():
    ...
    failed_notifications = Notification.objects.filter(status="failed").select_related(
        "anomaly__polygon_action__polygon", "target"
    )
    ...
    logger.info(
        f"Retried {retry_count} failed notifications, {success_count} successful"
    )
