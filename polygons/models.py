...
class Polygon(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="polygons"
    )
...
class NotificationTarget(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    polygon_action = models.ForeignKey(
        PolygonAction, on_delete=models.CASCADE, related_name="notification_targets"
    )
    target_type = models.CharField(
        max_length=20, choices=TARGET_TYPES, verbose_name="Тип цели"
    )
    target_value = models.CharField(
        max_length=500, verbose_name="Значение цели"
    )  # API key, device ID
...
class AnomalyDetection(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    polygon_action = models.ForeignKey(
        PolygonAction, on_delete=models.CASCADE, related_name="anomalies"
    )

    anomaly_type = models.CharField(
        max_length=50, choices=ANOMALY_TYPES, verbose_name="Тип аномалии"
    )
...
class Notification(models.Model):
    ...
    anomaly = models.ForeignKey(
        AnomalyDetection, on_delete=models.CASCADE, related_name="notifications"
    )
    target = models.ForeignKey(
        NotificationTarget, on_delete=models.CASCADE, related_name="notifications"
    )
