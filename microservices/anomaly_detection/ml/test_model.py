...
def prepare_test_data_from_csv(csv_path: str, window_size: int = 24, global_mean: np.ndarray = None, global_std: np.ndarray = None):
    ...
    for i, device_id in enumerate(devices):
        ...
        if (i + 1) % 50 == 0:
            logger.info(f"  Processed {i + 1}/{len(devices)} devices...")
    ...
    logger.info(f"Created {len(data)} test samples, shape: {data.shape}")
    logger.info(f"Anomaly windows: {labels.sum()} / {len(labels)}")
...
def test_model():
    ...
    if not Path(test_path).exists():
        ...
        logger.info("Please run: python data/gen_data.py")
        return
    ...
    try:
        ...
        if 'normalization' in metadata:
            ...
            logger.warning("Warning: No normalization stats in metadata, using local normalization")
    except Exception as e:
        logger.warning(f"Warning: Could not load metadata: {e}")
        n_features = 17
        threshold = 0.1
        threshold_95 = 0.05
    ...
    if data.size == 0:
        logger.error("No test data created")
        return
    ...
    logger.info(f"\nOverall Statistics:")
    logger.info(f"  Total windows: {len(scores)}")
    logger.info(f"  Mean score: {scores.mean():.4f}")
    logger.info(f"  Std score: {scores.std():.4f}")
    logger.info(f"  Min score: {scores.min():.4f}")
    logger.info(f"  Max score: {scores.max():.4f}")
    logger.info(f"  Threshold (95%): {threshold_95:.4f}")
    logger.info(f"  Threshold (99%): {threshold:.4f}")
    ...
    for atype, results in anomaly_type_results.items():
        ...
        logger.info(f"  {atype}:")
        logger.info(f"    Total: {results['total']}, Detected: {results['detected']}, Rate: {detection_rate:.1%}")
    ...
    if len(normal_scores) > 0:
        ...
        logger.info(f"  Total normal windows: {len(normal_scores)}")
        logger.info(f"  Mean score: {normal_scores.mean():.4f}")
        logger.info(f"  False positive rate: {false_positives / max(1, len(normal_scores)):.1%}")
    ...
    logger.info(f"Plot saved to: models/test_results.png")
    ...
    if anomaly_type_results:
        ...
        logger.info(f"Additional plot saved to: models/anomaly_scores_by_type.png")

    logger.info("\n" + "="*60)
    logger.info("SUMMARY")
    logger.info("="*60)
    logger.info(f"Total anomaly windows: {labels.sum()}")
    logger.info(f"Detected anomalies: {true_positives}")
    logger.info(f"Detection rate: {recall:.1%}")
    logger.info(f"False alarm rate: {false_positives / max(1, (~labels).sum()):.1%}")
