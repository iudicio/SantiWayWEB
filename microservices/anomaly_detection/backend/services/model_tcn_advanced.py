...
def load_advanced_model(
    ...
    if Path(model_path).exists():
        try:
            ...
            model.eval()
            logger.info(f"Advanced model loaded from {model_path}")
        except Exception as e:
            logger.warning(f"Warning: Failed to load model from {model_path}: {e}")
    else:
...
if __name__ == "__main__":
    ...
    logger.info(f"Anomaly scores shape: {scores.shape}")
