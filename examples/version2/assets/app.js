    function showTab(id){
      ...
      if(id==='Dashboard'){ ensureDashboard(); setTimeout(()=> map && map.invalidateSize(), 50); }
    }
    ...
    function regenerateData(){
      state.devices = generateDevices(500);
    ...
    function ensureDashboard(){
      ...
      if(!state.devices.length){ regenerateData(); }
    ...
    function renderTable(){
      ...
      sh.textContent = `Отображено ${state.filtered.length ? start+1 : 0} до ${end} из ${state.filtered.length} записей`;
