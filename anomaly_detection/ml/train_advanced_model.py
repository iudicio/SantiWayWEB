...
async def prepare_training_data_extended(
    ...
    logger.info(f"Fetching data from ClickHouse (use_extended_features={use_extended_features})...")
    ...
    logger.info(f"Computing features for {min(100, len(devices))} devices...")
    ...
    for i, device_id in enumerate(devices[:100]):
        ...
        if (i + 1) % 20 == 0:
            logger.info(f"  Processed {i + 1}/{min(100, len(devices))} devices...")
    ...
    logger.info(f"Number of features: {len(global_mean)}")
    ...
    logger.info(f"Created {len(data)} training samples, shape: {data.shape}")
...
async def main():
    ...
    try:
        ...
        if data.size == 0:
            logger.warning("No training data available. Please ingest data first.")
            return

        n_features = data.shape[2]
        logger.info(f"\n{'='*60}")
        logger.info(f"Number of features: {n_features}")
        logger.info(f"Extended features: {args.use_extended}")
        ...
        logger.info(f"  Hidden channels: {settings.HIDDEN_CHANNELS}")
        logger.info(f"  Kernel size: {settings.KERNEL_SIZE}")
        logger.info(f"  Dropout: {settings.DROPOUT}")
        logger.info(f"  Attention heads: {settings.NUM_ATTENTION_HEADS if args.use_attention else 'None'}")
        ...
        logger.info(f"Model saved to: models/tcn_advanced_model.pt")
        logger.info(f"Metadata saved to: models/model_metadata_advanced.json")
