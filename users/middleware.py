...
class RedirectToLoginMiddleware(MiddlewareMixin):
    ...
    def __init__(self, get_response):
        ...
        # Разрешенные url
        self.exempt_url_names = getattr(settings, 'AUTH_EXEMPT_URL_NAMES', [
            'users:login',
            'users:registration'
        ])
    ...
    def __call__(self, request):
        ...
        if resolved_url and  (resolved_url.url_name in self.exempt_url_names or
                              f"{resolved_url.app_name}:{resolved_url.url_name}" in self.exempt_url_names):
            return self.get_response(request)
