...
class BackendAnomalyDetector:
    ...
    def _validate_and_clean_data(
        self, X: Union[pd.DataFrame, np.ndarray], is_training: bool = False
    ) -> tuple:
        ...
        if isinstance(X, pd.DataFrame):
            ...
            if self.feature_names is not None:
                ...
                raise ValueError(
                    "Модель обучена на ndarray без feature_names. "
            ...
            if (
                ...
                and self.feature_names_policy == "strict"
            ):
                raise ValueError(
                    "Модель обучена на DataFrame. "
        ...
        if not is_training and self.is_fitted and self.n_features_ is not None:
            if X_values.shape[1] != self.n_features_:
                if self.feature_names_policy == "strict":
                    raise ValueError(
                        f"Ожидается {self.n_features_} признаков, "
                        f"получено {X_values.shape[1]}"
                    )
                else:
                    self.logger.warning(
                        f"Изменилось количество признаков: "
        ...
        try:
            ...
            if isinstance(X, np.ndarray):
                if X.ndim != 2 or X.shape[0] == 0 or X.shape[1] == 0:
                    raise ValueError(
                        "Ожидается непустой 2D ndarray (n_samples x n_features)"
                    )
            elif isinstance(X, pd.DataFrame):
                if X.empty or X.shape[1] == 0:
                    raise ValueError(
                        "DataFrame не может быть пустым и должен содержать признаки"
                    )
            else:
                raise TypeError(
                    "Поддерживаются только pandas DataFrame и numpy ndarray"
                )
            ...
            if isinstance(X, pd.DataFrame):
                ...
                if self.enable_warnings and self.feature_names_policy == "strict":
                    self.logger.warning(
                        "Обучение на ndarray без feature_names. "
            ...
            if validation_split > 0:
                ...
                self.logger.info(
                    f"Детерминированное разделение данных (seed={self.random_state}): "
            ...
            self.logger.info(
                f"Модель обучена на {X_clean.shape[0]} образцах "
    ...
    def _validate_model(self, X_val: np.ndarray):
        ...
        try:
            ...
            self.logger.info(
                f"Валидация: доля аномалий {anomaly_rate:.3f}, "
    ...
    def _calibrate_thresholds(self):
        ...
        for p in percentiles:
            self.severity_quantiles[f"q{p:02d}"] = float(
                np.percentile(self.train_scores, p)
            )
        ...
        if len(anomaly_scores) > 0:
            ...
            self.logger.warning(
                "Нет аномалий в обучающих данных. " "Используем статистические пороги."
            )

        # Сохраняем границы для нормализации скоров
        score_min, score_max = float(np.min(self.train_scores)), float(
            np.max(self.train_scores)
        )
        self.score_bounds = {
            "min": score_min,
            "max": score_max,
    ...
    def _validate_threshold(self, threshold: float) -> bool:
        ...
        if self.train_scores is not None:
            ...
            train_mean, train_std = np.mean(self.train_scores), np.std(
                self.train_scores
            )

            reasonable_min = min(
                train_min - abs(train_min) * 0.5, train_mean - 3 * train_std
            )
            reasonable_max = max(
                train_max + abs(train_max) * 0.5, train_mean + 3 * train_std
            )

            if threshold < reasonable_min or threshold > reasonable_max:
                if self.enable_warnings:
                    self.logger.warning(
                        f"Threshold {threshold:.3f} вне разумного диапазона "
            ...
            if threshold < -5 or threshold > 5:
                if self.enable_warnings:
                    self.logger.warning(
                        f"Threshold {threshold} вне общего диапазона [-5, 5]"
                    )
                return False
    ...
    def predict_anomalies(
        self, X: Union[pd.DataFrame, np.ndarray], threshold: Optional[float] = None
    ) -> np.ndarray:
        """
    ...
    def predict_batch(self, X: Union[pd.DataFrame, np.ndarray]) -> List[Dict[str, Any]]:
        ...
        self.logger.info(
            f"Батчевая обработка {len(X)} образцов "
        ...
        for i in range(0, len(X), self.batch_size):
            ...
            if i % (self.batch_size * 10) == 0:
                self.logger.debug(
                    f"Обработано {min(i + self.batch_size, len(X))} "
                    f"из {len(X)} образцов"
                )
    ...
    def get_anomaly_details(
        ...
        results = []
        for i, (pred, score, proba) in enumerate(
            zip(predictions, scores, anomaly_proba)
        ):
            is_anomaly = pred == -1
    ...
    def get_detection_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "anomaly_rate": (
                current_stats["anomaly_count"] / current_stats["prediction_count"]
                if current_stats["prediction_count"] > 0
                else 0.0
            ),
            "last_prediction": (
                current_stats["last_prediction_time"].isoformat()
                if current_stats["last_prediction_time"]
                else None
            ),
            ...
            stats.update(
                {
                    ...
                    "training_anomaly_rate": float(
                        np.sum(self.train_scores < 0) / len(self.train_scores)
                    ),
                    "score_distribution": {
                        "mean": float(np.mean(self.train_scores)),
                        "std": float(np.std(self.train_scores)),
    ...
    def load_model(self, filepath: str, strict_version_check: bool = True):
        ...
        try:
            ...
            if (
                ...
                and saved_sklearn_version != current_sklearn_version
            ):
            ...
            self.logger.info(
                f"Модель загружена из {filepath}, создана: {created_at}, "
    ...
    def export_to_onnx(self, onnx_path: str, meta_path: str):
        ...
        metadata = {
            ...
            "feature_names": self.feature_names,
            "severity_thresholds": self.severity_thresholds
            or {"CRITICAL": -0.65, "HIGH": -0.45, "MEDIUM": -0.2},
            "severity_quantiles": self.severity_quantiles,
            "score_bounds": self.score_bounds
            or {"mean": 0.0, "std": 1.0, "min": -1.0, "max": 1.0, "range": 2.0},
            "class_version": self.class_version,
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"BackendAnomalyDetector(contamination="
